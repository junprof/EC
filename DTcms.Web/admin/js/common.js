...
function openwin(title, url, winwidth, winheight, imgurl) {
...
function openwin_msg(title, url) {
    if (url != null && url != "") {
        layer.alert(title, {
...
function jsalert(msgtitle) {
    var laindex = layer.alert(msgtitle, {
        skin: 'layui-layer-molv' //样式类名
        , closeBtn: 0
    }, function () {
        layer.close(laindex);
...
//可以自动关闭的提示，基于artdialog插件
function jsprint_s(msgtitle, url, callback) {

    parent.jsalert(msgtitle);
    if (url == "back") {
        frames["mainframe"].history.back(-1);
    } else if (url != "") {
        window.location.href = url;
