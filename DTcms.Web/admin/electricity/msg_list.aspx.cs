...
{
    public partial class msg_list : ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("msg_list", DTEnums.ActionEnum.View.ToString()); //检查权限
                Model.manager model = GetAdminInfo(); //取得当前管理员信息
                RptBind(" m.user_id="+ model.id + " ", "m.addtime desc");
            }
        }
        ...
        protected string CombSqlTxt(string _keywords)
        {
            ...
            {
                strTemp.Append(" and (i.onenetnum =  '" + _keywords + "' or i.name like '%" + _keywords + "%')");
            }
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("msg_list_page_size", "DTcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("msg_list_page_size", "DTcmsPage", _pagesize.ToString(), 14400);
                }
            }
